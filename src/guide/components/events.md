...
Незважаючи на те, що це необов’язково, рекомендується визначати всі випущені події, щоб краще задокументувати, як має працювати компонент. Це також дозволяє Vue виключати відомих слухачів із [fallthrough attributes](/guide/components/attrs.html#v-on-listener-inheritance), уникаючи крайніх випадків, спричинених подіями DOM, які вручну випромінюються стороннім кодом.
...
Щоб це справді спрацювало, компонент `<Custom Input>` має виконати дві дії:

1. Прив’язати атрибут `value` рідного елемента `<input>` до властивості `modelValue`
...
Інший спосіб реалізації `v-моделі` в цьому компоненті полягає у використанні властивості `computed`, доступної для запису, як з геттером, так і з сеттером. Метод `get` має повертати властивість `modelValue`, а метод `set` має видавати відповідну подію:
...
За замовчуванням `v-model` для компонента використовує `modelValue` як властивість і `update:modelValue` як подію. Ми можемо змінити ці імена, передавши аргумент `v-model`:

```vue-html
<MyComponent v-model:title="bookTitle" />
```

У цьому випадку дочірній компонент має очікувати властивість `title` і видавати подію `update:title` для оновлення батьківського значення:
...
Модифікатори, додані до компонента `v-model`, будуть надані компоненту через властивість `modelModifiers`. У наведеному нижче прикладі ми створили компонент, який містить властивість `modelModifiers`, яка за умовчанням має порожній об’єкт:
...
Зауважте, що властивість `modelModifiers` компонента містить `capitalize`, а його значення — `true`, оскільки воно  прив’язано до `v-model` `v-model.capitalize="myText"`.
